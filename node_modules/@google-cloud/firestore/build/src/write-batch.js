...
class WriteBatch {
     ...
     * delete.
     * @param {Timestamp=} precondition.lastUpdatedTime If set, enforces that the
 ...
 * and 'lastUpdatedTime' use valid types.
 *
 * @private
 * @internal
...
function validatePrecondition(arg, value, options) {
    ...
    if (precondition.lastUpdatedTime !== undefined) {
        ++conditions;
        if (!(precondition.lastUpdatedTime instanceof timestamp_1.Timestamp)) {
            throw new Error(`${(0, validate_1.invalidArgumentMessage)(arg, 'precondition')} "lastUpdatedTime" is not a Firestore Timestamp.`);
        }
    }
    if (conditions > 1) {
