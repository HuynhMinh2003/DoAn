...
export class FileWatcherCertificateProvider implements CertificateProvider {
  ...
  private maybeStartWatchingFiles() {
    if (!this.refreshTimer) {
       ...
       * used and unused. */
      const timeSincelastUpdated = this.lastUpdatedTime ? (new Date()).getTime() - this.lastUpdatedTime.getTime() : Infinity;
      if (timeSincelastUpdated > this.config.refreshIntervalMs) {
        this.updateCertificates();
      }
      if (timeSincelastUpdated > this.config.refreshIntervalMs * 2) {
