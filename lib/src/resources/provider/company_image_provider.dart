...
class CompanyImageProvider extends ChangeNotifier {
  ...
  void fetchAvatar() {
    ...
    DocumentReference docRef = FirebaseFirestore.instance.collection("companies").doc(userId);
  ...
  Future<String?> uploadSelectedImageAndGetUrl(String userId, String uniqueFileName) async {
    try {
      final storageRef = FirebaseStorage.instance
          .ref()
          .child('companies/$userId/$uniqueFileName'); // Sử dụng tên file duy nhất
  ...
  Future<String?> uploadSelectedImageAndGetUrl1(
    ...
    try {
      ...
      final storageRef = FirebaseStorage.instance
          .ref()
          .child('companies/$userId/$uniqueFileName');
  ...
  Future<void> loadImage() async {
    try {
      ...
      final snapshot = await FirebaseFirestore.instance.collection('companies').doc(user.uid).get();
  ...
  Future<void> loadImageByCompanyId(String companyId) async {
    try {
      final snapshot = await FirebaseFirestore.instance.collection('companies').doc(companyId).get();
