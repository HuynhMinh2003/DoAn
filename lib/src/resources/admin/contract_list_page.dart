...
import 'package:do_an/src/resources/admin/add_resident_page.dart';
...
import 'contract_list_mobile_page.dart'
    if (dart.library.html) 'contract_list_web_page.dart';
...
class _ContractListPageState extends State<ContractListPage> {
  ...
  Map<String, bool> hasActiveContract = {};
  ...
  void didChangeDependencies() {
    ...
    contractNotifier = Provider.of<ContractNotifierProvider>(context, listen: true);
                  ...
                  onPressed: selectedIdToRemove == null
                    ...
                    try {
                      ...
                      if (isRepresentative) {
                        ...
                        if (others.isEmpty) {
                          ...
                          await showDialog(
                            ...
                            if (newId != null) {
                              ...
                              // 1. Create reference to the resident's contractHistory subcollection
                              final residentHistoryRef = FirebaseFirestore.instance
                                  .collection("residents")
                                  .doc(selectedIdToRemove)
                                  .collection("contractHistory");

                              // 2. Query: filter by contractId, sort by joinedAt descending (newest first), limit 1
                              final residentHistorySnapshot = await residentHistoryRef
                                  .where("contractId", isEqualTo: currentContractId)
                                  .orderBy("joinedAt", descending: true)  // newest → oldest
                                  .limit(1)                                // only 1 record
                                  .get();

                              // 3. If exists, update leftAt for that latest record
                        ...
                        // Update leftAt in resident's contractHistory (if entry with matching contractId exists)
  ...
  Future<void> showApartmentContractInfoDialog(BuildContext context, Apartment apartment, ContractData contract, VoidCallback onRefresh) async {
    final apartmentDocRef =
        FirebaseFirestore.instance.collection("apartments").doc(apartment.id);
          ...
          actions: [
            OutlinedButton(
              onPressed: () async {
                ...
                try {
                  // 1. Update contract status to inactive
                  await contractDoc.reference.update({'isActive': false});

                  // 2. Record contract termination history
                  ...
                  // 3. Update residents: isExit, lastUpdated and leftAt in contractHistory subcollection
                  final residentsSnapshot = await residentsRef
                      .where('apartmentId', isEqualTo: apartment.id)
                      .get();
                  for (var residentDoc in residentsSnapshot.docs) {
                    // 3.1 update isExit and lastUpdated
                    await residentDoc.reference.update({
                      'isExit': true,
                      'leaveAt': Timestamp.now(),
                    });

                    // 3.2 update leftAt in the latest contractHistory
            ...
            OutlinedButton(
              onPressed: () async {
                ...
                try {
                  // 3. Update endDate and lastUpdated
                  ...
                  // 7. Close original dialog and refresh UI
                  if (context.mounted) {
                    Navigator.pop(context);
                    onRefresh();
                  }
