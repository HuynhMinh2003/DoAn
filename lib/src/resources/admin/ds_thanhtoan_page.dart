            ...
            child: Column(
              ...
              children: [
                ...
                if (latestPayment != null) ...[
                  Divider(),
                  Text(
                    "Hóa đơn tháng (${latestPayment['month']}):",
                    style: TextStyle(
                        fontSize: 4.sp,
                        fontWeight: FontWeight.bold,
                  ...
                  SizedBox(height: 8.h),
                  Text("Tiền quản lý: ${latestPayment['managementFee']} đ", style: TextStyle(fontSize: 3.5.sp)),
                  Text("Tiền nước: ${latestPayment['waterFee']} đ", style: TextStyle(fontSize: 3.5.sp)),
                  Text("Tiền gửi xe: ${latestPayment['parkingFee']} đ", style: TextStyle(fontSize: 3.5.sp)),
                  Text("Tổng: ${latestPayment['total']} đ", style: TextStyle(fontSize: 3.5.sp)),
                  Text("Trạng thái: ${latestPayment['status']}", style: TextStyle(fontSize: 3.5.sp)),
                  if (latestPayment['status'] == 'Chưa thanh toán đủ' && latestPayment['debt'] != null)
                    Text("Còn thiếu: ${latestPayment['debt']} đ", style: TextStyle(fontSize: 3.5.sp, color: Colors.red)),
                ],
              ],
            ),
          ),
          actions: [
            ...
            if (latestPayment != null) ...[
              TextButton(
                ...
                child: Text("Xác nhận đã thanh toán", style: TextStyle(fontSize: 3.sp)),
              ),
              TextButton(
                onPressed: () {
                  showDialog(
                    ...
                    builder: (_) {
                      ...
                      return AlertDialog(
                        title: Text("Nhập số tiền còn thiếu", style: TextStyle(fontSize: 5.sp)),
                        content: TextField(
                        ...
                        actions: [
                          ...
                          TextButton(
                            onPressed: () async {
                              ...
                              if (text.isEmpty || debt == null || debt <= 0) {
                                // Hiển thị cảnh báo nếu chưa nhập hoặc nhập sai
                                showDialog(
                                  context: context,
                                  builder: (_) => AlertDialog(
                                    title: Text("Lỗi", style: TextStyle(fontSize: 5.sp)),
                                    content: Text("Vui lòng nhập số tiền nợ hợp lệ (lớn hơn 0).", style: TextStyle(fontSize: 4.sp)),
                                    actions: [
                                      TextButton(
                                        onPressed: () => Navigator.pop(context),
                ...
                child: Text("Xác nhận chưa đủ (ghi nợ)", style: TextStyle(fontSize: 3.sp)),
              ),
            ]
          ],
        ),
    ...
    try {
      ...
      if (allPayments.isEmpty) {
        showDialog(
          context: context,
          builder: (_) => AlertDialog(
            title: Center(child:Text("Lịch sử thanh toán", style: TextStyle(fontSize: 5.sp)),),
      ...
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text("Lịch sử thanh toán", style: TextStyle(fontSize: 5.sp)),
          content: SizedBox(
