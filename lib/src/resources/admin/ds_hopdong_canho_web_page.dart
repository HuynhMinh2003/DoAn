...
Future<void> exportContractApartmentsToExcel(List<Apartment> apartments) async {
  try {
    ...
    for (final apartment in apartments) {
      print('Đang xử lý căn hộ: ${apartment.apartmentName}');
      ...
      if (contractId != null) {
        ...
        if (contractData != null) {
          ...
          print('Không tìm thấy dữ liệu hợp đồng cho: ${apartment.id}');
        }
      } else {
        print('Căn hộ ${apartment.id} chưa có hợp đồng.');
      }
    }
...
// Hàm định dạng Timestamp → String (dd/MM/yyyy)
